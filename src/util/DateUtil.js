...
class DateUtil {
  ...
  expired = date => {
    const m = moment(date, 'YYYY-MM-DD h:mm:ss')
  ...
  getDifferenceInDaysWithoutHours = (date1, date2) => {
    return this.getDifferenceInDays(this.removeHours(date1), this.removeHours(date2))
  }
  ...
  findDuration = (earlierDate, laterDate) => {
    ...
    if (oDiff.years > 0) {
      if (oDiff.years === 1) {
        ...
        concatDiff.push(this.createTimeObject(oDiff.years + ' anos', oDiff.years, 1))
      }
    }
    if (oDiff.months > 0) {
      if (oDiff.months > 10) {
      ...
      if (oDiff.months === 1) {
        ...
        concatDiff.push(this.createTimeObject(oDiff.months + ' meses', oDiff.months, 2))
      }
    }
    if (oDiff.days > 0 && concatDiff.length < 2) {
      ...
      if (oDiff.days === 1) {
        ...
        concatDiff.push(this.createTimeObject(oDiff.days + ' dias', oDiff.days, 3, 5))
      }
    }
    if (oDiff.hours > 0 && concatDiff.length < 2) {
      ...
      if (oDiff.hours === 1) {
        ...
        concatDiff.push(this.createTimeObject(oDiff.hours + ' horas', oDiff.hours, 4, 5))
      }
    }
    if (oDiff.minutes > 0 && concatDiff.length < 2) {
      ...
      if (oDiff.minutes === 1) {
        ...
        concatDiff.push(this.createTimeObject(oDiff.minutes + ' minutos', oDiff.minutes, 5, 10))
      }
    }
